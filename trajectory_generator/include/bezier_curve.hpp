  ...
  bool getCurveVelocity(T u, T * ret)
  {
    if (u > _end_time) {
      ...
      for (int i(0); i < DIM; ++i) {
        ret[i] = 0.;
        ret[i] +=
          _coeff[0] * (-(NUM_CTR_PT - 1) * pow(1 - u, (NUM_CTR_PT - 1) - 1)) * _CtrlPt[0][i];
        ...
        ret[i] += _coeff[NUM_CTR_PT - 1] * (NUM_CTR_PT - 1) * pow(u, NUM_CTR_PT - 2) *
                  _CtrlPt[NUM_CTR_PT - 1][i];
